...
**资金利用率 / 收益 / 回撤 前沿（¥20 万账户，2018.6–2026.4，约 7.8 年）：**

| 额度利用率 | 总盈利 | 总收益率 | 简单年化 | 最大回撤 |
|---|---|---|---|---|
| 50%（保守） | ¥115,939 | 58% | **7.4%** | 14.5% |
| 100%（满额~1x） | ¥270,103 | 135% | **17.3%** | 32.5% |
| 150% | ¥388,876 | 194% | 24.9% | 39.4% |
| 200%（激进） | ¥554,776 | 277% | 35.5% | 49.2% |

![真实资金净值](assets/spread_money_equity.png)

**结论（真金白银）**：¥20 万账户、满额度（约 1x）下 7.8 年赚约 **¥27 万、年化约 17%、最大回撤约 33%**；
保守减半到 50% 额度则年化约 7%、回撤约 15%。**年均约 70 次交易（4 对合计）**，频率不高、可手动跟。

> 注意：高胜率(64-71%)≠低回撤——亏损单是 4σ 止损的大单，且会成簇出现（关系阶段性失效），
> 所以满额度仍有 33% 回撤。这正是套利"高胜率、肥尾"的真实形态，别被胜率迷惑而过度加仓。
...
**交易规则（4 个套利对各自独立执行）**
1. **开仓**：Z 上穿 +1.5 → 做空价差（卖 B 买 A）；Z 下穿 −1.5 → 做多价差（买 B 卖 A）。次日开盘或当日尾盘按对冲手数建仓。
2. **止盈**：Z 回到 ±0.3 以内 → 平仓。
...
**资金与风控**
- 保守：每对只用 50% 额度（≈历史年化 7%、回撤 15%）；进取：满额度（≈年化 17%、回撤 33%）。
