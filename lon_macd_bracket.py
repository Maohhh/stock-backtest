...
def main(argv):
    ...
    if "FG0" in detail:
        ...
        be_ok = r["win_rate"] > be
        R.append(
            f"- 盈亏比 {tp/sl:.2g}:1，盈亏平衡需胜率 {be*100:.1f}%；实际胜率 {r['win_rate']*100:.1f}% "
            f"{'高于' if be_ok else '低于'}盈亏平衡，**净期望 {r['exp_net_pts']:+.1f} 点/笔（{r['exp_net_pct']*100:+.3f}%）**。\n"
        )
        if be_ok:
            R.append(f"- 这次是**正期望**：盈亏比 {tp/sl:.2g}:1 把盈亏平衡线压到 {be*100:.0f}%，进场只要不太差就能过线。\n")
        else:
            R.append(
                f"- 仍是**负期望**：止盈 {tp:g} / 止损 {sl:g} 点把盈亏平衡线抬到 {be*100:.0f}%，"
                f"但进场短期偏随机，实际胜率只有 {r['win_rate']*100:.0f}%，过不了线——"
                "胜率升上去了，可输的那几笔是赢的两倍大，照样填不平。\n"
            )
